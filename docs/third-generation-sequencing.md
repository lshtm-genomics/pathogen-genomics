...
```
freebayes -f reference.fasta SRR26779702.bam  -F 0.8 | bcftools view -Oz -o SRR26779702.vcf.gz
```

The `-f` flag specifies the reference genome to use, and the `-F` flag specifies the minimum allele frequency to call a variant. In this case, we are using a threshold of 0.8, which means that any position with an allele frequency below 0.8 will be ignored. The `bcftools view` command is used to convert the output of freebayes to a VCF file, which is a standard format for storing variant data.
